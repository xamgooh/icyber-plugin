...
class ComparisonShortcode
{
	...
	public function __construct()
	{
		...
		add_shortcode('Comparison', array($this, 'custom_shortcode'));
	...
	public function custom_shortcode($atts)
	{
		...
		if (is_array($category) && count($category) > 1 && $filter) {
			...
			foreach ($category as $key => $value) {
				...
				$wp_query = $this->get_comarison_posts([0 => $value], $attributes['max']);

				$hidden = !($key === array_key_first($category)) ? " hidden" : "";
				$html .= '<div class="com_comarison__list--container ' . $random_id . '_' . $value . $hidden . '">';
				$html .= $this->comparion_list_html->get_list_html($wp_query);
				if ($wp_query->found_posts > $attributes['max'] && $attributes['load_more']) {
					$html .= '<button data-restUrl="' . site_url() . '" data-list="' . $value . '" data-limit="' . $attributes['max'] . '" class="com_btn--loadmore">' . get_option('comporisons_filter_label') . '</button>';
				}

				$html .= '</div>';
			}
		} else {
			$wp_query = $this->get_comarison_posts($category, $attributes['max']);
			$html .= '<div class="com_comarison__list--container ' . $random_id . '_' . $random_id . '">';
			$html .= $this->comparion_list_html->get_list_html($wp_query, $attributes['max']);
			if ($wp_query->found_posts > $attributes['max'] && $attributes['load_more']) {
				if (is_array($category)) {
					$html .= '<button data-restUrl="' . site_url() . '" data-list="' . str_replace(' ', '-', mb_strtolower(trim($category[0]))) . '" data-limit="' . $attributes['max'] . '" class="com_btn--loadmore">' . get_option('comporisons_filter_label') . '</button>';
				} else {
					$html .= '<button data-restUrl="' . site_url() . '" class="com_btn--loadmore">' . get_option('comporisons_filter_label') . '</button>';
				}
			}
			$html .= '</div>';
		}
